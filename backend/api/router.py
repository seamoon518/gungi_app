...
from api.schemas import MoveRequest, ValidMovesResponse, ArataRequest, ValidArataResponse
...
def valid_moves(game_id: str, row: int, col: int) -> ValidMovesResponse:
    ...
    options = get_valid_moves(state.board, row, col)
...
def valid_arata(game_id: str) -> ValidArataResponse:
    state = _get_or_404(game_id)
    positions = get_valid_arata_positions(state.board, state.current_player)
